...
get_token() {
  curl -s -X POST \
    ...
    "$BASE/auth/login" | grep -o '"token":"[^"]*"' | cut -d'"' -f4
}
...
echo "=== AUTH ==="
CEO_TOKEN=$(get_token "ceo@benchmark.com")
[ -n "$CEO_TOKEN" ] && echo "  PASS CEO login" && PASS=$((PASS+1)) || { echo "  FAIL CEO login"; FAIL=$((FAIL+1)); }

WK_TOKEN=$(get_token "drawer1@benchmark.com")
...
echo ""
echo "=== MONTH LOCKS ==="
t GET "/month-locks/1" 200 "Month locks" "$CEO_TOKEN"
